...
def plot_lda_3d(lda_model, corpus, file_paths):
    ...
    colors = ['r', 'g', 'b']
    labels = file_paths  # Label each point with its respective file name
    
    for i, vec in enumerate(topic_matrix_3d):
        ax.scatter(vec[0], vec[1], vec[2], color=colors[i % len(colors)], label=labels[i])
...
def main():
    # List of file paths
    file_paths = ['trans_rights.txt', 'abortion.txt', 'veganism.txt']
